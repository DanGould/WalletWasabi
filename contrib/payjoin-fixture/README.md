...
(upstream ask tracked in the payjoin meta repo, snags §W3).
...
nix develop /home/claude-agent/payjoin/rust-payjoin#csharp --command \
  cargo build --manifest-path contrib/payjoin-fixture/Cargo.toml
```

## Dependency pinning

`Cargo.toml` uses a **path dependency** on the pinned rust-payjoin worktree
(`rust-payjoin-wt-wasabi-ffi`, upstream master `d27b7b137c9e8696ad6bf542ba0c1bf93665df72`)
that sits as a sibling of this repository's parent directory — the dev-reproducible form
for the machine this harness runs on. If this branch travels anywhere else, swap to the
git form (one-line change, documented inline in Cargo.toml):
