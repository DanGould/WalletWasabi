...
public partial class SendViewModel : RoutableViewModel
{
	...
	public SendViewModel(UiContext uiContext, IWalletModel walletModel, SendFlowModel parameters, ShowQrCodeCameraDialog showQrCodeCameraDialog) : base(uiContext)
	{
		...
		this.WhenAnyValue(x => x.To)
			...
			.Subscribe(x =>
			{
				// A To rewrite from within the parse must not re-trigger it: re-parsing the
				// rewritten bare address would undo the state the parse just established — in
				// particular PayJoinEndPoint, which the parser clears to null by default.
	...
	private bool TryParseUrl(string? text)
	{
		...
		// The To rewrite mid-parse raises ValidateToField before the parse has finished
		// setting the remaining fields, so it can validate against half-updated state.
		// Re-run it against the settled state and let NextCommand's canExecute pick it up.
