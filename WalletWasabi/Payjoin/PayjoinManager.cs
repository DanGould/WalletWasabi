...
public class PayjoinManager : PeriodicRunner
{
	...
	public PayjoinManager(
		...
		WabiSabi.Client.WalletProvider getWallets,
	...
	private readonly WabiSabi.Client.WalletProvider _getWalletsAsync;
	...
	private async Task ContributeInputsAsync(PayjoinSessionRecord session, Wallet wallet, SqliteReceiverSessionPersister persister, WantsInputs state, CancellationToken cancel)
	{
		...
		// the same coin (BBM pitfall: concurrent payjoins reusing one input).
	...
	{
		using var keysClient = new OhttpKeysClient(new System.Uri(relay));
		return await keysClient.GetOhttpKeysAsync(new System.Uri(_configuration.DirectoryUrl), cancellationToken).ConfigureAwait(false);
	}
